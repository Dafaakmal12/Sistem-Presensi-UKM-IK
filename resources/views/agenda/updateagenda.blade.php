...
@section('content')
    <h1 class="text-4xl font-bold my-4">Mengupdate Agenda</h1>
    
    <form action="{{ route('agenda.updatelistagenda', $agenda->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="mb-6">
            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Agenda</label>
            <input type="name" id="name" name="nama" value="{{ $agenda->nama }}"
        ...
        <div class="mb-6">
            ...
            <input type="tempat" id="tempat" name="tempat" value="{{ $agenda->tempat }}"
                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                placeholder="name@flowbite.com" required>
        </div>
        <div class="mb-6">
